...
-- Regression: an Anthropic session must not route background/default
-- resolutions to openai-codex (and print OAuth errors) just because
-- settings.defaults.provider = openai-codex. resolve_route now gates the
-- implicit fallback on provider_has_auth, mirroring pi's hasConfiguredAuth.
local reg = require("psi.api_registry")

-- Deterministic auth state: codex unconfigured, anthropic configured.
...
-- Simulate settings.defaults.provider = openai-codex via the settings module.
