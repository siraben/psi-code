...
local function events(tail)
  local out = {
    { key = "paste-start" }, { key = "text", text = payload }, { key = "paste-end" },
  }
  for _, event in ipairs(tail) do out[#out + 1] = event end
  return out
end
local undo_ok = true
if #kb.keys("tui.editor.undo") > 0 then
  local undo = rt._debug_edit_keys("", 0, events({{key="backspace"},{key="ctrl--"}}), false)
...
if probe.input == "draft" then
  local draft = rt._debug_edit_keys("", 0, events({{key="ctrl-p"},{key="escape"}}), false, {busy=true})
