...
return table.concat({
  ...
  fmt(tui.handle_key({key="alt-enter", busy=true, input_length=1})),
  ...
  fmt(tui.handle_key({key="ctrl-d", busy=true, input_length=1})),
  fmt(tui.handle_key({key="alt-up", busy=true, input_length=0, queue_count=2})),
