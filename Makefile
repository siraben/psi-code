...
LOCAL_CPPFLAGS = -Iinclude $(shell $(PKG_CONFIG) --cflags chibi-scheme libcjson)
LOCAL_LDFLAGS = $(shell $(PKG_CONFIG) --libs chibi-scheme libcjson)
...
$(BUILD_DIR)/main.o: src/main.c include/psi/common.h include/psi/runtime.h include/psi/session.h include/psi/vm.h
	$(CC) $(CPPFLAGS) $(LOCAL_CPPFLAGS) -DPSI_SCHEME_BOOT_FILE=\"$(SCHEME_BOOT_FILE)\" $(BASE_CFLAGS) $(CFLAGS) -c $< -o $@
