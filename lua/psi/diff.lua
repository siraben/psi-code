...
local function common_prefix_length(xs, ys)
  ...
  local count = 0
  for i = 1, n do
    if xs[i] == ys[i] then count = count + 1 else break end
  end
  return count
end
...
local function common_suffix_length(xs, ys, prefix_count)
  ...
  local count = 0
  while count < math.min(xlen, ylen) - prefix_count
    and xs[xlen - count] == ys[ylen - count] do
    count = count + 1
  end
  return count
end
...
local function context_lines(lines, prefix)
  local out = {}
  for _, line in ipairs(lines) do out[#out + 1] = prefix .. line end
  return out
end
...
function M.colored_diff(before_text, after_text)
  ...
  for _, l in ipairs(limit_lines(before_middle, 40)) do rendered[#rendered+1] = ansi.red("- " .. l) end
  for _, l in ipairs(limit_lines(after_middle, 40))  do rendered[#rendered+1] = ansi.green("+ " .. l) end
  for _, l in ipairs(context_lines(after_ctx, ansi.dim("  "))) do rendered[#rendered+1] = l end

  if #rendered == 0 then return ansi.dim("  no visible diff") end
