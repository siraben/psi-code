...
encode_value = function(v)
  ...
  if tv == "table" then
    local out = {}
    if is_array(v) then
      for i = 1, #v do out[#out + 1] = encode_value(v[i]) end
...
local function decoder(text)
  ...
  local function err(msg) error("json decode error at byte " .. tostring(i) .. ": " .. msg, 0) end
  local function peek() return text:sub(i, i) end
  ...
  local function parse_string()
    if peek() ~= '"' then err("expected string") end
    i = i + 1
    local out = {}
    while i <= n do
      ...
      elseif c == "\\" then
        ...
        elseif e == "u" then
          ...
          if not hex:match("^%x%x%x%x$") then err("bad unicode escape") end
  ...
  local function parse_number()
    ...
    if not s then err("expected number") end
  ...
  local function parse_array()
    ...
    i = i + 1
    skip_ws()
    if peek() == "]" then i = i + 1; return setmetatable(out, { __jsontype = "array" }) end
    while true do
      out[#out + 1] = parse_value()
      skip_ws()
      local c = peek()
      if c == "]" then i = i + 1; return setmetatable(out, { __jsontype = "array" }) end
      if c ~= "," then err("expected comma or ]") end
      i = i + 1
      skip_ws()
    end
  end
  ...
  local function parse_object()
    ...
    i = i + 1
    skip_ws()
    if peek() == "}" then i = i + 1; return out end
    while true do
      local key = parse_string()
      skip_ws()
      if peek() ~= ":" then err("expected colon") end
      i = i + 1
      out[key] = parse_value()
      skip_ws()
      local c = peek()
  ...
  skip_ws()
  if i <= n then err("trailing input") end
  return value
end
