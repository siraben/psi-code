...
| --- | --- | --- |
| Central session/runtime object | `packages/coding-agent/src/core/agent-session.ts` | Partial. `psi` exposes a small C observer/runtime bridge plus Lua `psi.agent_session`; it does not have one `AgentSession`-equivalent object that owns every mode. |
