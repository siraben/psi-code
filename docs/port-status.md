...
| Session persistence | `packages/coding-agent/docs/session.md` | Ported to pi-style v3 JSONL schema: typed entries, parent pointers, cache markers, file-op provenance, custom entries, custom model-context messages, UTF-16 surrogate sanitising. Lua keeps the file's entry tree and can rebuild the active path from `parentId` links. |
