...
| MCP | Intentionally omitted from pi core; extensions/packages may provide it | psi deliberately differs: a small Lua JSON-RPC client supports MCP tools over Streamable HTTP because psi already requires libcurl. It negotiates capabilities and sessions, discovers/registers namespaced tools, and shares the normal tool/scheduler/abort paths. There is no bundled stdio MCP client; the `MCP` build gate exposes only low-level process primitives. |
| Compaction and summaries | `packages/coding-agent/src/core/compaction/` | Ported. Manual and configurable token-aware auto-compaction runs before prompts and after complete turns across every provider, with one compact-and-retry attempt for provider context overflows. Cut points are selected before summary generation, split turns receive a separate prefix summary, structured tool/thinking content is serialized, and file-op provenance accumulates in model-visible summaries. Compaction appends a compacted active branch without dropping sibling branches. `/tree <id> --summarize` can summarize the abandoned branch and attach a `branch_summary` entry at the destination. Extension customization/cancel hooks are not ported. |
