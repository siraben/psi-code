...
| `psi.tui_layout.set_prompt_max_rows(rows_or_nil)` | Override the target visible multiline prompt height from Lua. Pass a number to set the row cap, or `nil` to clear the override. The TUI runtime may still supply `tui.prompt.max_rows` from settings; frame mode applies a separate physical-space cap while chat mode uses the terminal height. |
