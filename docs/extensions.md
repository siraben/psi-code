...
```
/help  /hotkeys  /quit (+ /q, :quit, :q)  /session  /status  /mcp
/system-prompt  /vim
...
| `psi.process_begin_stdio_argv(argv)` / `psi.process_write(handle, data)` / `psi.process_try_write(handle, data)` / `psi.process_poll(handle, ms)` / `psi.process_close_stdin(handle)` / `psi.process_terminate(handle)` / `psi.process_finish(handle)` | Bidirectional stdio process primitive used by the MCP client. `process_write` blocks until the full payload is accepted; `process_try_write` makes one non-blocking attempt and returns the byte count actually accepted (0 on EAGAIN), so callers can drive the retry loop and yield through `psi.sched.sleep_ms` to keep the TUI responsive while a slow child drains stdin. Prefer higher-level tools unless you are implementing a protocol bridge. |
