...
| `/quit` | `/q, :quit, :q` | — | Exit the shell |
| `/session` | — | — | Show current session info |
| `/new` | `/clear` | — | Start a fresh session in place |
| `/resume` | — | `<path>` | Load a session file from disk |
| `/import` | — | `<path>` | Import a JSONL session |
| `/name` | — | `<text>` | Set the session display name |
| `/model` | — | `<spec>` | Switch model mid-session |
| `/set` | — | `<setting> <value>` | Set runtime options |
| `/thinking` | — | `<level>` | Set reasoning level |
| `/login` | — | `<provider>` | Authenticate an OAuth provider |
| `/copy` | — | — | Copy the last assistant message to the clipboard |
| `/export` | — | `[path]` | Write the session as markdown |
| `/compact` | — | `[N]` | Summarize older context, keeping recent messages |
| `/fork` | — | `[N]` | Save the first N entries to a new session file |
| `/clone` | — | `[path]` | Duplicate the current session at its current position |
| `/branch` | — | `[entry-id]` | Show the session tree or switch the active branch leaf |
| `/branches` | — | — | Show the session tree |
| `/reload` | — | — | Reload extensions, prompt templates, and keybindings |
| `/rainbow` | — | — | Show all 256 terminal background colors |
| `/queue` | — | `[list|clear|drop N|edit N text]` | Inspect or edit queued messages |
...
The table above is regenerated by `make docs`; do not edit it by hand.
...
Default action map (regenerated from `DEFINITIONS` in
`lua/psi/keybindings.lua` by `make docs`):
...
| `tui.editor.cursorWordLeft` | `alt-b` | Move cursor word left |
| `tui.editor.cursorWordRight` | `alt-f` | Move cursor word right |
| `tui.editor.cursorLineStart` | `home`, `ctrl-a` | Move to line start |
| `tui.editor.cursorLineEnd` | `end`, `ctrl-e` | Move to line end |
| `tui.transcript.lineUp` | `up` | Scroll transcript up |
| `tui.transcript.lineDown` | `down` | Scroll transcript down |
| `tui.transcript.pageUp` | `page-up`, `ctrl-u` | Scroll transcript page up |
| `tui.transcript.pageDown` | `page-down` | Scroll transcript page down |
| `tui.queue.previous` | `ctrl-p` | Show previous queued message |
| `tui.queue.next` | `ctrl-n` | Show next queued message |
| `tui.queue.restore` | `up` | Edit queued message |
...
| `tui.input.clear` | — | Delete to start of prompt |
| `tui.input.reverseSearch` | `ctrl-r` | Reverse-search prompt history |
| `tui.editor.deleteCharBackward` | `backspace` | Delete character backward |
| `tui.editor.deleteCharForward` | `delete` | Delete character forward |
| `tui.editor.deleteWordBackward` | `ctrl-w`, `alt-backspace` | Delete word backward |
| `tui.editor.deleteWordForward` | `alt-d` | Delete word forward |
| `tui.editor.deleteToLineEnd` | `ctrl-k` | Delete to end of prompt |

**Other**

| Action | Default keys | Description |
|---|---|---|
| `app.interrupt` | `ctrl-g` | Abort current turn |
| `app.exit` | `ctrl-d` | Exit when prompt is empty |
| `app.redraw` | `ctrl-l` | Redraw screen |
