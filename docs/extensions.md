...
| `psi.tempfile_path([prefix])` | Creates a private temp file under `$TMPDIR` (or `/tmp`) and returns its path. The bash tool uses this for spillover when output exceeds the in-memory truncation cap. |
