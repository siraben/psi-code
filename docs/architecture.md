...
The full set of `psi.*` host primitives exposed to Lua (regenerated by
`make docs` from `PSI_REG` calls in `src/lua/vm.c`):

<!-- @generated:host-primitives -->
`psi.abort_reset` · `psi.abort_trigger` · `psi.add_history`  
`psi.current_date` · `psi.cwd` · `psi.embedded_doc`  
`psi.embedded_doc_names` · `psi.embedded_source` · `psi.embedded_source_names`  
`psi.file_append` · `psi.file_exists` · `psi.file_type`  
`psi.file_write` · `psi.file_write_atomic` · `psi.file_write_secure`  
`psi.host_tick` · `psi.http_get` · `psi.http_post`  
`psi.http_stream_begin` · `psi.http_stream_finish` · `psi.http_stream_poll`  
`psi.is_aborted` · `psi.json_decode` · `psi.json_encode`  
`psi.list_dir` · `psi.log` · `psi.mkdir_p`  
`psi.mkdir_parent` · `psi.parent_directory` · `psi.path_expand`  
`psi.path_join` · `psi.path_resolve` · `psi.process_begin`  
`psi.process_begin_argv` · `psi.process_finish` · `psi.process_poll`  
`psi.process_run` · `psi.process_run_argv` · `psi.read_file`  
`psi.read_file_slice` · `psi.readline` · `psi.runtime_info`  
`psi.session_append` · `psi.session_clear` · `psi.session_id`  
`psi.session_keep_recent_by_tokens` · `psi.session_message_count` · `psi.session_messages`  
`psi.session_messages_from` · `psi.session_parent_id` · `psi.session_path`  
`psi.session_set_id` · `psi.session_set_parent_id` · `psi.session_set_path`  
`psi.session_token_estimate_from` · `psi.set_usage` · `psi.sleep_ms`  
`psi.stdout_write` · `psi.tempfile_path` · `psi.time_ms`  
`psi.tool_call` · `psi.tool_progress` · `psi.tui_clear`  
`psi.tui_draw_line` · `psi.tui_draw_raw_line` · `psi.tui_poll_key`  
`psi.tui_refresh` · `psi.tui_render_frame` · `psi.tui_set_cursor`  
`psi.tui_set_tick_handler` · `psi.tui_set_tool_progress_handler` · `psi.tui_size`  
`psi.tui_suspend` · `psi.version`
<!-- @end -->

CLI flags accepted by the entry point (regenerated from
`src/runtime/cli.c`):
