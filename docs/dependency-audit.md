...
Runtime findings affect release decisions because those libraries are in the final `psi` closure. Build-time findings are tracked for builder and CI hygiene, but they do not automatically block a runtime release unless the vulnerable build tool processes untrusted input or affects build integrity.
...
The app writes CycloneDX, SPDX, CSV, no-heuristic-CPE, vulnerability, and triage artifacts under `sbom/`. These generated artifacts are ignored by git; keep durable audit policy in `docs/dependency-audit.md` and `sbom/vulnxscan.whitelist.csv`, and publish full SBOM output as CI or release artifacts when needed.
