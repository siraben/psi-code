 ...
 *
 * Implementation derived from Markus Kuhn's mk_wcwidth, released by
 * the author into the public domain. The original announcement and
 * source live at https://www.cl.cam.ac.uk/~mgk25/ucs/wcwidth.c. The
 * tables below mirror that file's intent (zero-width combining /
 * format characters and East Asian Wide / Fullwidth ranges) updated
 * to cover the BMP + supplementary plane ranges that show up in
 * everyday text and the common emoji blocks.
 *
 * Kept C89-clean (no //-comments, no compound literals, no VLAs) so
 * the file builds under the same `-std=c89 -pedantic -Werror` flags
 * as the rest of psi's core.
 */
...
/*
 * Sorted, non-overlapping ranges of zero-width characters: combining
 * marks, format controls, variation selectors, ZWJ/ZWNJ, etc.
 */
static const struct psi_wcwidth_range psi_wcwidth_zero[] = {{0x00AD, 0x00AD}, {0x0300, 0x036F},
    ...
    {0x0859, 0x085B}, {0x08D3, 0x08E1}, {0x08E3, 0x0902}, {0x093A, 0x093A}, {0x093C, 0x093C},
    {0x0941, 0x0948}, {0x094D, 0x094D}, {0x0951, 0x0957}, {0x0962, 0x0963}, {0x0981, 0x0981},
    {0x09BC, 0x09BC}, {0x09C1, 0x09C4}, {0x09CD, 0x09CD}, {0x09E2, 0x09E3}, {0x0A01, 0x0A02},
    ...
    {0x0B3F, 0x0B3F}, {0x0B41, 0x0B44}, {0x0B4D, 0x0B4D}, {0x0B56, 0x0B56}, {0x0B62, 0x0B63},
    {0x0B82, 0x0B82}, {0x0BC0, 0x0BC0}, {0x0BCD, 0x0BCD}, {0x0C00, 0x0C00}, {0x0C3E, 0x0C40},
    ...
    {0x0D01, 0x0D01}, {0x0D41, 0x0D44}, {0x0D4D, 0x0D4D}, {0x0D62, 0x0D63}, {0x0DCA, 0x0DCA},
    ...
    {0x17B7, 0x17BD}, {0x17C6, 0x17C6}, {0x17C9, 0x17D3}, {0x17DD, 0x17DD}, {0x180B, 0x180E},
    ...
    {0x1AB0, 0x1AFF}, {0x1B00, 0x1B03}, {0x1B34, 0x1B34}, {0x1B36, 0x1B3A}, {0x1B3C, 0x1B3C},
    ...
    {0x202A, 0x202E}, {0x2060, 0x2064}, {0x2066, 0x206F}, {0x20D0, 0x20FF}, {0x2CEF, 0x2CF1},
    ...
    {0xA80B, 0xA80B}, {0xA825, 0xA826}, {0xA8C4, 0xA8C5}, {0xA8E0, 0xA8F1}, {0xA926, 0xA92D},
    {0xA947, 0xA951}, {0xA980, 0xA982}, {0xA9B3, 0xA9B3}, {0xA9B6, 0xA9B9}, {0xA9BC, 0xA9BC},
    ...
    {0x1171D, 0x1171F}, {0x11722, 0x11725}, {0x11727, 0x1172B}, {0x16AF0, 0x16AF4},
...
/*
 * Sorted, non-overlapping ranges of double-width characters: East
 * Asian Wide, Fullwidth, and the emoji ranges that render as 2 cells
 * in a typical terminal.
 */
static const struct psi_wcwidth_range psi_wcwidth_double[] = {{0x1100, 0x115F}, {0x231A, 0x231B},
    {0x2329, 0x232A}, {0x23E9, 0x23EC}, {0x23F0, 0x23F0}, {0x23F3, 0x23F3}, {0x25FD, 0x25FE},
    ...
    {0x16FE0, 0x16FE4}, {0x16FF0, 0x16FF1}, {0x17000, 0x18AFF}, {0x1B000, 0x1B12F},
    {0x1B150, 0x1B152}, {0x1B164, 0x1B167}, {0x1B170, 0x1B2FF}, {0x1F004, 0x1F004},
...
static int psi_wcwidth_in_table(const struct psi_wcwidth_range *table, int table_len, int cp) {
    int lo;
    int hi;
...
int psi_wcwidth(int cp) {
    ...
    zero_count = (int)(sizeof(psi_wcwidth_zero) / sizeof(psi_wcwidth_zero[0]));
    if (psi_wcwidth_in_table(psi_wcwidth_zero, zero_count, cp)) {
        return 0;
    }
    double_count = (int)(sizeof(psi_wcwidth_double) / sizeof(psi_wcwidth_double[0]));
    if (psi_wcwidth_in_table(psi_wcwidth_double, double_count, cp)) {
        return 2;
    }
    return 1;
}
