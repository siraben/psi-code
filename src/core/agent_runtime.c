...
int psi_agent_runtime_turn_with_observer(struct psi_agent_runtime *runtime, const char *user_text,
    ...
    status = psi_vm_run_agent_turn(&runtime->vm, user_text, observer, abort_signal, runtime->model,
        runtime->max_tokens, response_text);
...
int psi_agent_runtime_compact(struct psi_agent_runtime *runtime, size_t keep_recent,
    ...
    return psi_vm_run_agent_compact(
        &runtime->vm, keep_recent, abort_signal, runtime->model, runtime->max_tokens, summary_text);
}
