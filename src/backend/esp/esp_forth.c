...
static const char BODY2[] =
    ") reachable on the local LAN. Tools available: system_info, wifi_scan, "
    "http_fetch (HTTPS via mbedTLS), gpio_mode/read/write, nvs_get/set, "
    "time_now, restart, uart_log. Prefer using tools to find ground truth "
    "instead of guessing. Keep replies short. [prompt built by zforth]";
...
zf_input_state zf_host_sys(zf_ctx *ctx, zf_syscall_id id, const char *input) {
    ...
    switch ((int)id) {
        case ZF_SYSCALL_EMIT:
            /* not used in our prompt-builder, but keep it sane */
            (void)zf_pop(ctx);
            break;
